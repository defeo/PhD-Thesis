...
Recall from Section~\ref{todo} that rational function reconstruction
takes as input a degree $n$ polynomial $T$, a polynomial $A$ of degree
less than $n$ and a target degree $m\le n$ and outputs the unique
...
Therefore, by choosing an appropriate $t\in O(\log_q p^k)$, C2-UD can
