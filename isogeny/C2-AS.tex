...
\begin{equation*}
  O\bigl((pd)^\omega i + {\sf PT}(i-1) + \Mult(p^{i+1}d)\log p\bigr)
\end{equation*}
...
\hyperref[todo]{fast Euclidean algorithm}.
...
then use \alg{Push-down} to obtain the corresponding polynomial in
$\F_q[X]$ and finally do a rational fraction reconstruction.

The first step costs $O\bigl(\Mult(p^{2k}d)\log p^k\bigr)$ using fast
techniques as in Section~\ref{todo}, then converting to $\F_q[c][X]$
takes $O\bigl(p^k{\sf L}(k-1)\bigr)$ and further converting to
$\F_q[X]$ takes $\Theta\bigl((pd)^2\bigr)$ by linear algebra. The
\hyperref[todo]{rational function reconstruction} then takes
$O\bigl(\Mult(p^kd)\log p^k\bigr)$.
