...
all the points of $E[p^k]$. We call C2-AS-FI the variant of C2-AS
resulting from applying this new algorithm.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE\label{alg:T:gal} compute $T_{i+1}^\sigma$  using \alg{IterFrobenius};
    \ENDFOR
    ...
    $\U_i[X]$ using \alg{Push-down}.
    \ENDFOR
  \end{algorithmic}
\end{algorithm}

For any $i>i_0$, step~\ref{alg:T:gal} of is repeated $p$ times, each
iteration taking $O\bigl(p^{k-i}{\sf L}(i-i_0)\bigr) \subset
O\bigl({\sf L}(k-i_0)\bigr)$ by Theorem~\ref{th:b-ifrob}.
Step~\ref{alg:T:prod} takes $O\bigl(\Mult(p^{k-i_0+1}d/r)\log p\bigr)$
using Algorithm~\ref{alg:subprod} and step~\ref{alg:T:push} takes
$O\bigl(p^{k-i+1}{\sf L}(i-i_0)\bigr) \subset O\bigl(p{\sf
  L}(k-i_0)\bigr)$.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \FOR {$i = k-1$ \TO $0$}
    \FORALL {$\sigma \in \Gal(\U_{i+1}/\U_i)$}
    \STATE\label{alg:A:gal} compute $P_{i+1}^\sigma$ using \alg{IterFrobenius};
    \ENDFOR
    \STATE\label{alg:T:lift} convert $T_i$ into an element of $\U_{i+1}$ using \alg{Lift-up}.
