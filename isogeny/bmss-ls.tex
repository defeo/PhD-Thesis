...
\begin{algorithm}
  \caption{\label{alg:bmss}\alg{BMSS}}
  \begin{algorithmic}[1]
...
$2^{\lceil\log_2(4\ell-1)\rceil}>p$, \hyperref[alg:bmss]{\alg{BMSS}}
...
\hyperref[alg:bmss]{\alg{BMSS}} cannot apply.
...
\hyperref[alg:bmss]{\alg{BMSS}}. The algorithm is summarized below; it
requires $p\ge5$ and it makes computations in an unramified extension
of degree $d$ of $\Q_p$, denoted by $\Q_q$.

\begin{algorithm*}
  \caption{\label{alg:le-si}\alg{Lercier-Sirvent}}
  \begin{algorithmic}[1]
    ...
    \STATE \label{alg:le-si:bmss}Apply \hyperref[alg:bmss]{\alg{BMSS}} to $\bar{E}$ and
...
\begin{nota}
  \pdfmctwo{Changed this note again.}  In the cases $p=2,3$, Elkies'
  ...
  However, in both cases we could not find a way to obtain the
  normalized models for $\bar{E}$ and $\bar{E}''$, like in
  Step~\ref{alg:le-si:elkies}. Furthermore, while for $p=3$ Vélu's
  formulas give an equation as simple as Eq.~\eqref{eq:166}, in the
  case $p=2$ we think that the equation satisfied by the isogeny is
  too complex to be solved efficiently.
\end{nota}
...
\begin{proof}
  ...
  q)$. \hyperref[alg:bmss]{\alg{BMSS}} takes $O(\Mult(\ell)\log\ell)$
