...
\label{sec:group-law}
\pdfmargincomment[color=red,subject=rev2]{I find both "chord-tangent"
...
\begin{definition}
  \pdfmargincomment[color=red,subject=rev2]{Yes, P2 has been (quickly) defined in Chap 1.}
...
\pdfmargincomment{Oups! Changed "Kummer surface" to "Kummer variety"
...
\pdfmargincomment[color=red,subject=rev2]{Not following Connell anymore: now a curve HAS a twist.}
An elliptic curve and a twist are isomorphic over the algebraic
closure $\clot{\K}$. The \index{twist!degree~of~a}degree of a twist is
the degree of the smallest extension $\K'/\K$ such that the two curves
...
\pdfmargincomment{Repetita iuvant: elliptic curves "over C".}
\begin{theorem}
...
\pdfmargincomment{It is the minimal polynomial "over C".}
...
The modular polynomial has $O(\ell^2)$ coefficients, and the
coefficients themselves have logarithmic height $O(\ell\log\ell)$. For
this reason minimal polynomials associated to other modular functions
are often preferred to $\Modpol_\ell$, as for example Atkin's
\emph{canonical polynomial} $\Modpol_\ell^c$~\cite{atkin88}.

Algorithms to compute the modular polynomial and its variants are
described in~\cite{morain95,sutherland10:modpol}.
