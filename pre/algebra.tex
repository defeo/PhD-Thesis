...
closure $\clot{\K}$. To any ideal $I$, we associate its \emph{set of
  zeros}
\begin{equation}
  \label{eq:215}
...
\index{algebraic~set!defined~over~a~field}\emph{defined over} $\K$
...
\index{variety!projective}\emph{projective variety} for homogeneous
ideals. The projective variety associate to the zero ideal is called
the \index{projective~space}\emph{projective space} of dimension $n$,
...
morphism) is
\index{rational~map!defined~over~a~field}\index{morphism!defined~over~a~field}\emph{defined
