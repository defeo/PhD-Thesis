...
\index{multiplication~function}\textit{multiplication
  function}\symb[Mn]{$\Mult(n)$}{Multiplication function},
...
$\ModComp(n)$\symb[Cn]{$\ModComp(n)$}{Modular composition
...
\index{discrete~Fourier~transform}\textit{discrete Fourier transform},
there are many algorithms of complexity $O(n\log n)$ to compute it,
they all go under the generic name of
\index{FFT}\index{fast~Fourier~transform}\textit{fast Fourier transform}
(FFT).

\pdfmcone{More bibliography around Schönage and Strassen.}
...
\index{formal~power~series}\textit{formal power series} on $R$. Its
...
monic, the \index{modular~composition}\textit{modular composition}
requires to compute
\begin{equation}
  \label{eq:190}
  f(g(X)) \mod h(X)
...
\index{formal~power~series!composition}\textit{composition of power
...
\index{pseudotrace}\textit{pseudotrace}\footnote{In~\cite{vzgathen+shoup92},
...
to construct the \index{subproduct~tree}\textit{subproduct tree}.
...
\index{Lagrange~interpolant}\textit{Lagrange interpolants} 
\begin{equation}
  \label{eq:223}
...
be the GCD of $f$ and $g$, a \index{Bézout~relation}\textit{Bézout
...
by the \index{extended Euclidean algorithm}\textit{extended Euclidean
  algorithm}.
...
\index{Padé~approximant}\textit{Padé approximant} of type
...
multiplication by \index{Kronecker~substitution}\textit{Kronecker
...
\index{transposed~algorithm}\textit{transposed algorithms}. The theory
...
\index{transposition~principle}\textit{transposition
...
\index{transposed~modular~reduction}\textit{transposed modular
...
\index{transposed~modular~multiplication}\textit{transposed modular
