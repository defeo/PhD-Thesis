...
modular compositions via square-and-multiply as follows.
...
We can apply the same idea to compute the $n$-th pseudotrace in time
$O(\ModComp(d)\log n)$, but we have to use a dynamic programming
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE $\Theta_o \la 0$;
    \STATE $\Theta_1 \la a\circ\Phi_{1}$;
    \STATE $k=b_0$;
