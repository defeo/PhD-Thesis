...
\pdfmargincomment[color=red,subject=rev2]{More entries in the matrix.}
...
algorithm \hyperref[alg:approximateas]{\alg{ApproximateAS}} finds the required solution.
...
\begin{algorithm}
  \caption{\label{alg:approximateas}\alg{ApproximateAS}} 
  \begin{algorithmic}[1]
...
\begin{theorem}
  ...
  Algorithm \hyperref[alg:approximateas]{\alg{ApproximateAS}} is correct and takes time $O(\Lift(i))$.
\end{theorem}
...
\begin{algorithm}
  \caption{\label{alg:artin-schreier}\alg{Artin-Schreier}}
  \begin{algorithmic}[1]
...
\begin{theorem}\label{theo:AS}
  Algorithm \hyperref[alg:artin-schreier]{\alg{Artin-Schreier}} is
...
\begin{algorithm}
  \caption{\label{alg:applyisomorphism}\alg{ApplyIsomorphism}} 
  \begin{algorithmic}[1]
...
of \hyperref[alg:rur]{\alg{RUR}} in the extension $\U_i= \U_{i-1}[X_i]/(P_i)$, that we
...
\begin{algorithm}
  \caption{\label{alg:applyinverse}\alg{ApplyInverse}} 
  \begin{algorithmic}[1]
...
\begin{proposition}\label{Prop:apply}
  Algorithms \hyperref[alg:applyisomorphism]{\alg{ApplyIsomorphism}} and \hyperref[alg:applyinverse]{\alg{ApplyInverse}} are correct
...
\begin{proof}
  ...
  operations: for \hyperref[alg:applyisomorphism]{\alg{ApplyIsomorphism}}, these are $p$
  multiplications / additions in the bivariate basis $\basis{D}_i$ of
  Section~\ref{sec:level-embedding}; for \hyperref[alg:applyinverse]{\alg{ApplyInverse}}, this is
...
representation. Unlike \hyperref[alg:liftup]{\alg{Lift-up}}, this algorithm will not make use
...
\hyperref[alg:rur]{\alg{RUR}}, we need to compute $t\cdot\rho(1)$. By
...
in step~\ref{alg:rur:5} of \hyperref[alg:rur]{\alg{RUR}}. Since $P'_i$ is Artin-Schreier,
...
\begin{nota}
  ...
  overall complexity of \hyperref[alg:applyinverse]{\alg{ApplyInverse}}.
\end{nota}
