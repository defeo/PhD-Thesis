...
$i=0$ and apply~\cite[Algorithm~5.2]{vzGS92}\todo. We summarize the
two variants in one unique algorithm \alg{IterFrobenius}.
...
\begin{theorem}
  ...
  correctly computes $v^{p^n}$ and takes time $O((i-j)\Lift(i))$.
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  $O(p^i\ModComp(d)\log^2 (n) + i \Lift(i))$ by
  Theorem~\ref{th:l-ifrob}. This gives $\Ptr(i,n) = \Ptr(i,\lfloor
  n/2\rfloor) +O(p^i\ModComp(d)\log^2 (n) + i \Lift(i))$, and thus
...
\begin{theorem}
  ...
  $\Ptr(i)=O((pi+\log (d))i\Lift(i)+p^i\ModComp(d)\log^2 (d))$ for $j \le
  i$.
\end{theorem}
\begin{proof}
  Correctness is clear. For the cost analysis, we write $\Ptr(i,j)$
  for the cost on input $i$ and $j$, so theorem~\ref{th:l-pseudo}
  gives $\Ptr(i,0)=O(p^i\ModComp(d)\log^2 (d) + i \Lift(i)\log
  (d))$. For $j>0$, step \ref{alg:pseudo:rec} costs $\Ptr(i,j-1)$,
  step~\ref{alg:pseudo:frob} costs $O(p i \Lift(i))$ by
  Theorem~\ref{th:b-ifrob} and step~\ref{alg:pseudo:sum} costs
  $O(p^{i+1}d)$. This gives $\Ptr(i,j) = \Ptr(i,j-1) +O(p i
  \Lift(i))$, and thus $\Ptr(i,j) \in O(pij{\sf L}(i) + \Ptr(i,0))$.
\end{proof}
...
$\PTr_{n}(\gamma_i)$ in time $O(p^i\ModComp(d)\log^2 d + i
\Lift(i)\log d)$. Same goes for the precomputation of all
