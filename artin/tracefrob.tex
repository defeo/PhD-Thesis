...
\hyperref[alg:iterfrobenius]{\alg{IterFrobenius}}.

\begin{algorithm}
  \caption{\label{alg:iterfrobenius}\alg{IterFrobenius}} 
  \begin{algorithmic}[1]
    ...
    \STATE \label{alg:frob:push} let $v_0 + v_1 x_i + \dots + v_{p-1} x_i^{p-1}=\text{\hyperref[alg:push-down]{\alg{Push-down}}}(v)$;
    ...
    \STATE \label{alg:frob:lift} return $\text{\hyperref[alg:liftup]{\alg{Lift-up}}}(F)$.
  \end{algorithmic}
\end{algorithm}
...
\begin{theorem}
  \label{th:b-ifrob}
  On input $v\wrt\U_i$ and $n=p^jd$, algorithm \hyperref[alg:iterfrobenius]{\alg{IterFrobenius}}
...
\begin{theorem}
  \label{th:l-ifrob}
  On input $v\wrt\U_i$ and $n<d$, algorithm \hyperref[alg:iterfrobenius]{\alg{IterFrobenius}}
  correctly computes $v^{p^n}$ and takes time 
  \begin{equation}
    \label{eq:94}
...
previous formulas. The first one, \hyperref[alg:littlepseudotrace]{\alg{LittlePseudotrace}}, is meant to
...
second one, \hyperref[alg:pseudotrace]{\alg{Pseudotrace}}, computes $\PTr_{p^jd}$ for $j>0$. It
...
\begin{algorithm}
  \caption{\label{alg:littlepseudotrace}\alg{LittlePseudotrace}}
  \begin{algorithmic}[1]
    ...
    \STATE \label{alg:lpseudo:rec} let $t=$ \alg{LittlePseudotrace}($v$,
    $i$, $m$);
    \STATE \label{alg:lpseudo:frob} let $t=t+$ \hyperref[alg:iterfrobenius]{\alg{IterFrobenius}}($t$, $i$, $m$);
    \STATE \label{alg:lpseudo:odd} if $n$ is odd, let $t=t+$ \hyperref[alg:iterfrobenius]{\alg{IterFrobenius}}($v$, $i$, $n$);
    \STATE return $t$.
  \end{algorithmic}
\end{algorithm}

\begin{algorithm}
  \caption{\label{alg:pseudotrace}\alg{Pseudotrace}}
  \begin{algorithmic}[1]
    \REQUIRE $v$, $i$, $j$ with $v\wrt\U_i$.
    \ENSURE $T_{p^jd}(v) \wrt \U_i$.
    \STATE \label{alg:pseudo:base} if $j = 0$ return \hyperref[alg:littlepseudotrace]{\alg{LittlePseudotrace}}($v$, $d$);
    \STATE \label{alg:pseudo:rec} $t_0=$\alg{Pseudotrace}($v, i, j-1$);
    \STATE \label{alg:pseudo:frob}for $h\in [1,\dots,p-1]$, let $t_h=\text{\hyperref[alg:iterfrobenius]{\alg{IterFrobenius}}}(t_{h-1}, i, j-1)$;
...
\begin{theorem}
  \label{th:l-pseudo}
  Algorithm \hyperref[alg:littlepseudotrace]{\alg{LittlePseudotrace}} is correct and takes time
  \begin{equation}
    \label{eq:96}
...
\begin{theorem}
  \label{th:b-pseudo}
  Algorithm \hyperref[alg:pseudotrace]{\alg{Pseudotrace}} is correct and takes time
  \begin{equation}
    \label{eq:97}
...
algorithm \hyperref[alg:littlepseudotrace]{\alg{LittlePseudotrace}}
makes less than $2\log d$ calls to
\alg{\hyperref[alg:iterfrobenius]{IterFrobenius}($x$,$i$,$n$)} for
...
using the \hyperref[alg:littlepseudotrace]{\alg{LittlePseudotrace}}
algorithm. The inner calls to
\hyperref[alg:iterfrobenius]{\alg{IterFrobenius}} only use
pseudotraces that are already known. Besides, a single call to
\hyperref[alg:littlepseudotrace]{\alg{LittlePseudotrace}}$(\gamma_i,i,d)$
...
the \hyperref[alg:pseudotrace]{\alg{Pseudotrace}} algorithm: this costs $\Ptr(i)$. Observe that in
total we only store $O(k^2 + k\log d)$ elements of the tower, thus the
space requirements are quasi-linear.

\begin{remark}
  A dynamic programming version of~\hyperref[alg:littlepseudotrace]{\alg{LittlePseudotrace}} as
