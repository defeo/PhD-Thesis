...
\begin{theorem}
  \label{th:l-pseudo}
  Algorithm \titleref{alg:littlepseudotrace} is correct and takes time
  \begin{equation}
    \label{eq:96}
...
\begin{theorem}
  \label{th:b-pseudo}
  Algorithm \titleref{alg:pseudotrace} is correct and takes time
  \begin{equation}
    \label{eq:97}
...
algorithm \titleref{alg:littlepseudotrace}
makes less than $2\log d$ calls to
\titleref{alg:iterfrobenius}$(x,i,n)$ for
...
using the \titleref{alg:littlepseudotrace}
algorithm. The inner calls to
\titleref{alg:iterfrobenius} only use
pseudotraces that are already known. Besides, a single call to
\titleref{alg:littlepseudotrace}$(\gamma_i,i,d)$
...
the \titleref{alg:pseudotrace} algorithm: this costs $\Ptr(i)$. Observe that in
