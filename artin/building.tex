...
\pdfmargincomment[color=red,subject=rev2]{Cited Bernstein.}
\hyperref[alg:compose]{\alg{Compose}} is a recursive process that cuts
...
\begin{algorithm}
  \caption{\label{alg:naivecompose}\alg{NaiveCompose}}
  \begin{algorithmic}[1]
...
\begin{lemma}
  \hyperref[alg:naivecompose]{\alg{NaiveCompose}} has cost $O(\deg(P)^2k\ell)$.  
\end{lemma}
...
\begin{algorithm}
  \caption{\label{alg:compose}\alg{Compose}}
  \begin{algorithmic}[1]
    \REQUIRE $P,R\in\F_p[X]$.
    \ENSURE $P(R)$.
...
\begin{theorem}
  ...
  $\deg(P)=s$, then \hyperref[alg:compose]{\alg{Compose}}$(P,R)$ outputs $P(R)$ in time $O(ps
  \log_p(s)k\ell)$.
\end{theorem}
\begin{proof}
  Correctness is clear, since $R^{p^n}=R(X^{p^n})$. To analyze the
  cost, we let $\sC(c,n)$ be the cost of {\hyperref[alg:compose]{\alg{Compose}}} when
  ...
  call to \hyperref[alg:compose]{\alg{Compose}} satisfy $cp^n\le s$ and $n\le\log_p (s)$; this
...
\begin{remark}
  ...
  \hyperref[alg:compose]{\alg{Compose}}, so by Theorem~\ref{theo:comp} the cost is $O(pd
  \log_p(d))$.
...
\hyperref[alg:compose]{\alg{Compose}} with arguments $Q_0$ and $R=X_1^p-X_1$; the cost is
...
\begin{algorithm}
  \caption{\label{alg:minimalpolynomial}\alg{MinimalPolynomial}}
  \begin{algorithmic}[1]
    \REQUIRE $Q_i$, $\Phi$.
    \ENSURE $Q_{i+1}$.
...
\begin{theorem}
  Algorithm \hyperref[alg:minimalpolynomial]{\alg{MinimalPolynomial}}
