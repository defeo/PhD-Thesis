...
\begin{proposition}\label{prop:pd}
  Algorithm \alg{Push-down} is correct and takes time 
  \begin{equation}
    \label{eq:92}
    O(p^{i+1}d
...
\begin{proof}
  ...
  \alg{Push-down-rec}, $\deg_Y(W) < (c+1)p^{n-1}$, so our call to
...
Section~\ref{sec:from-univ-bivar}, \alg{Push-down} is the same thing
as the map
\begin{equation}
  \label{eq:81}
  \begin{aligned}
...
and~\ref{alg:rur:5} of \alg{RUR}.  To compute the power projection we
...
\begin{proposition}\label{prop:lu}
  Algorithm \alg{Lift-up} is correct and takes time
  \begin{equation}
    \label{eq:85}
...
\begin{proof}
  ...
  \alg{TransposedMul} implements the transposed multiplication; an
  ...
  \alg{Push-down}. Then, the costs of steps~\ref{alg:lift-up:mult}
