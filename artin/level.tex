...
\alg{Lift-up}.  In other words, \alg{Push-down} inputs $v\wrt\U_i$ and
...
repeatedly applying \alg{Push-down}, and the opposite is obtained by
repeatedly applying \alg{Lift-up}.

Hereafter, we let $\Lift:\N-\{0\} \to \N$ be such that both
\alg{Push-down} and \alg{Lift-up} can be performed in time $\Lift(i)$; to
...
\begin{proof}
  ...
  \alg{Push-down-rec}, $\deg(W,Y) < (c+1)p^{n-1}$, so our call to
  ...
  In \alg{Push-down}, we have $cp^n< p^id$ and $n<\log_p (p^id)$, so
...
transpose of \alg{Push-down}.  As in
Section~\ref{sec:from-univ-bivar}, \alg{Push-down} is the map
\begin{equation}
  \label{eq:81}
  \begin{aligned}
...
In \alg{Push-down-rec}, we identify the output space $\F_p[Y,X_i]$,
...
simplification yields the pseudo-code we give.  In \alg{Push-down},
after calling \alg{Push-down-rec}, we evaluate $W$ at
...
Then, we use Theorem~\ref{th:rur} to write $v$ as a polynomial in
...
\alg{Push-down$^\ast$} as it was shown in the previous subsection. The
rest of the computation goes as in steps~\ref{alg:rur:4}
and~\ref{alg:rur:5} of \alg{Rational Univariate Representation}.
...
\end{proposition}
\begin{proof}
  Correctness is clear by the discussion above. \alg{TransposedMul}
  ...
  same cost as \alg{Push-down}. Then, the costs of
