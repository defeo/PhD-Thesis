...
remove any ambiguity about the fact that we are talking of left
modules.  We set $R^0=0$, the zero module, and we denote by $\bom$ (or
$\ket{\bom}$) its unique element.

The dual space $\dual{(R^n)}=\hom(R^n,R)$ has a natural structure of
right $R$-module (or of left $R^\op$-module) by the assignment
\begin{equation}
  \label{eq:232}
...
\hyperref[sec:linear-algebra:bra-ket]{brakets} to denote the natural
...
\begin{definition}[Arithmetic basis]
  An \index{arithmetic~basis}\textbf{arithmetic $R$-basis} is a (not
  necessarily finite) set of arithmetic operators over $R$.
\end{definition}

The arithmetic basis we will work with is the
\index{standard~left-linear~basis}\emph{standard left-linear basis},
...
\begin{definition}[Arithmetic node]
  Let $\mathcal{B}$ be an $R$-basis. A \index{node}\textbf{node} over
...
We draw input and output nodes in square boxes and evaluation nodes in
round boxes. Input and output nodes are ordered from left to right;
ports are not ordered because the basis is commutative.
...
them. Intuitively the semantic corresponds to recursively feed the
inputs to the nodes, evaluate $\beta(v)$ at the inputs and collect the
outputs.
...
\begin{equation}
  ...
  \begin{gathered}
    \begin{aligned}
      +=\dual{\hub} : \dual{R}\oplus \dual{R} &\ra \dual{R}\text{,}  &  \hub=\dual{+} : \dual{R} &\ra \dual{R}\oplus \dual{R}\text{,}\\
      \bra{a,b} &\mapsto \bra{a}+\bra{b}\text{,}\qquad   &      \bra{a} &\mapsto \bra{a,a}\text{,}\\
    \end{aligned}\\ \\
    \begin{aligned}
      \lmul{a}\eqdef\dual{(\rmul{a})} : \dual{R} &\ra \dual{R}\text{,} \\
                                             \bra{b} &\mapsto \bra{ab}\text{,}
    \end{aligned}\\ \\
    \begin{aligned}
...
In particular, this makes $(V',E^{-1})$ the reverse graph of $(V,E)$
in a graph-theoretic sense. Figure \ref{fig:circuit} shows two
circuits that are each other's dual (note that, since $\Z$ is in the
center of any ring, $\lmul{2}=\rmul{2}$). We can now state the
...
\end{lemma}
\begin{proof}
  We start by proving that for any evaluation node $v$ and any edge
  $e$ stemming from $v$
  \begin{equation}
    \label{eq:250}
  ...
  Let $e_1\le_v\ldots\le_ve_k$ be the edges incident to $v$, and let
  $v_1,\ldots,v_k$ be the corresponding nodes. Then, by definition
  \begin{equation}
    \label{eq:251}
  ...
  \begin{equation}
    \label{eq:254}
    \pi_j\circ\eval_C = \sum_{i=1}\sum_{p\in x_i\leadsto y_j}\eval_p\circ\pi_i
    \text{,}
  \end{equation}
...
\begin{corollary}
  ...
  by arithmetic circuits of same sizes and depths. In particular if
  $C$ computes $f$ and $\dual{C}$ computes $\dual{f}$,
  \begin{gather*}
...
\begin{remark}
  ...
  this case $\dual{(\rmul{a})}=\rmul{\bar{a}}$). This case of the
  transposition theorem (applied to unitary matrices) is well known in
  quantum physics and in particular in quantum computing.
\end{remark}
...
\begin{nota}
  ...
  only showed the case
  $R=\C$. Some~\cite{burgisser+clausen-shokrollahi,bostan+lecerf+schost:tellegen}
  attribute the discovery to tellegen, Bordweijk's advisor, but this
  is debated~\cite{djb:tellegen}.
  ...
  Kaltofen and Yagati~\cite{kaltofen+lakshman89} pointed out that the
  transposition theorem can be proven as a special case of Baur and
  Strassen's differentiation of circuits~\cite{baur+strassen83}. We
  shall come back to this question in
  Section~\ref{sec:word-about-automatic}.
\end{nota}
...
generalizes to them. We will not study uniform circuit families more
in depth; what we shall do instead, is directly work on computer programs
implicitly representing circuit families and automatically deduce the
transposed family without actually using the circuit model. More
details on uniform circuit families can be found in \cite{vollmer}.
