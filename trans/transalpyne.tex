...
future. \tALpy{} relies on python's operator overloading to represent
ring operations.
...
same as python's. A \tALpy{} file contains a \emph{type declaration}
...
and \lstinline+if+s. The syntax is identical to python's.
...
\item Function calls.
\end{itemize}
The syntax for all of these is identical to python's. The only notable
